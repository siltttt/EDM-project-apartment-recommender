...
st.subheader('Características de los mejores pisos')

for cont,i in enumerate(recommended_pisos):
    st.markdown(f'**{cont+1}. {pisos["Dirección"][i]}**')
    if pisos['Rebajado%'][i]>0:
        st.write(f':green[Este piso ha sido rebajado un {pisos["Rebajado%"][i]}%]')
    st.write('🔹 Precio:', str(pisos['Precio'][i]).replace(',','.'), '€')
    st.write('🔹 Superficie:', str(pisos['m2'][i]).replace(',','.'), 'm²')
    st.write('🔹 Habitaciones:', str(pisos['Habitaciones'][i]))
    st.write('🔹 Baños:', str(pisos['Baños'][i]))
    if pisos['Ascensor'][i]>0:
